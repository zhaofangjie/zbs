...
function MsgAutoScroll(){
	...
	if($('#publicChat').find("div").length>100){$('#publicChat').find("div:eq(0)").remove();}
...
UserList=(function(){
	...
	return{
		...
		init:function(){
			list['ALL']={sex:2,chatid:'ALL',nick:'大家'};
			var request_url='/index/ajax/getrlist?rid='+My.rid+'&r='+RoomInfo.r+'&'+Math.random() * 10000;
		...
		showuser:function(u){
			...
			var uimg=u.mood;
			if(u.color=="0")uimg='/style/face/p1/null.gif';
			...
			li.innerHTML='<a href="javascript:void(0)"  title="'+u.nick+'">'
				...
				+'<span class="uimg"><img src="'+uimg+'" onerror="this.src=\'/style/face/p1/null.gif\'" border="0" class="head" /></span>'
...
function showsyssmg(txt){
	//alert(txt);
	var date= Datetime(0);
	var s='<div class="chat-item notmine system"><div class="user-img"> <div class="uimg"><img src="/style/face/sys.gif"></div><div class="gimg"></div></div><div class="chat-info"> <div class="chat-info-1"> <span class="chat-name">系统消息</span> <span class="chat-time">'+date+'</span></div><div class="chat-info-2"> <div class="chat-msg">'+txt+'</div></div></div></div>';
...
function getsysmsg(){
	$.getJSON("/index/ajax/getsysmsg","rid="+My.rid,
		function(data){
...
function Mkick(adminid,rid,kid,cause)
{
    $.getJSON('/index/ajax/kick?kid='+kid,'',function(re){
        if(re.state=='1'){
            if(re.ban=='1'){
                location.href="/room/error.php?msg="+encodeURI('你被踢出！'+re.losttime+'前禁止进入房间！<br>原因是 '+re.sn+'');
            }
            else{
                //location.href="/room/error.php?msg="+encodeURI('你被踢出！原因是 '+re.sn+'');
                $("#video-win").prepend('<div style="position: relative; width: 100%; height: 100%; background: #000; color: #fff; padding-top:150px;line-height:30px;font-size:20px;">您好，您的试听时间已结束，请点击右上角免费注册，可继续收听！</div>');
            }
        }
    });
}
	...
	if(Txt.indexOf('C0MMAND')!=-1)
	{
		...
		switch(command[1])
		{
			...
			case 'rebotmsg':
				...
				msg.Style=Msg.Style;
				var s='<div class="chat-item notmine" ><div class="user-img"> <div class="uimg"><img src="/index/ajax/getImgFace?t=p1&u='+msg.ChatId+'"/></div><div class="gimg"><img src="'+msg.ico+'"/></div></div><div class="chat-info"> <div class="chat-info-1"> <span class="chat-name"  onclick="ToUser.set(\''+msg.ChatId+'\',\''+msg.nick+'\')">'+msg.nick+'</span> <span class="chat-time">'+date+'</span></div><div class="chat-info-2"> <div class="chat-msg">'+msg.Txt+'</div></div></div></div>';
				MsgShow(s,0);
				break;
	...
	else
	{
		...
		if(RoomInfo.msgAudit=="1"){
			...
			if(check_auth('msg_audit')){
				msgAuditBt=" <a href='javascript:void(0)' onclick='bt_msgAudit(\""+msgid+"\",this)'  id='bt_audit_"+msgid+"'><img src='/style/room/images/22.png' style='border:0px;' title='审核通过'></a>";
				msgAuditShow="";
			}
...
function Init(){
	...
	$(".am-tabs li").click(function() {
		...
		if($(this).index()==0){
			...
			$("#footer").hide();
			$(".div_plus").hide();
			$(".tabsnav-"+$(this).data("showtab")).height($(window).outerHeight(true)-$("#header").outerHeight(true)-$("#head_1").outerHeight(true));
		}
	});
}
...
function getHongBao(hid){
	...
	$.getJSON("/index/ajax/gethongbao?hid="+hid,function(re){
...
function bt_msgBlock(id){
	if(id!=""){
		...
		$.ajax({type: 'get',url: '/index/ajax/msgblock?st=1&msgid='+id});
	}
}
function bt_msgAudit(id,a){
	SysSend.command('msgAudit',id);
	$(a).hide();
	$.ajax({type: 'get',url: '/index/ajax/msgblock&st=0?msgid='+id});
}
function uploadAvatar(ElementId, editor) {
	//上传文件
	$.ajaxFileUpload({
		url:'/style/upload/upload.php',//处理图片脚本
		secureuri :false,
...
function qiandao(){
	if(My.color=="0"){layer.open({
		...
		return;}
	$.getJSON("/index/ajax/qiandao?rid="+My.rid,function(re){
		if(re.code==1){
			layer.open({
...
function openHbdiv(){
	...
	layer.open({
		...
		,anim: 'up'
		,style: 'position:fixed; bottom:0; left:0; width: 100%; height: 307px; padding:0 0 10px 0; border:none;border-radius:5px 5px 0px 0px'
	});
}
...
function sendGifts(e){
	...
	if(sid&&nick&&num&&gid){
		layer.closeAll();
		$.get("/index/ajax/sendgift?sid="+sid+"&num="+num+"&gid="+gid,function(re){
			if(re=="0"){
				layer.open({
					content: '错误！没有该礼物数据！'
					,skin: 'msg'
...
function sendHb(me,ne,txt){
	...
	getMyMoney($(me).val(),$(ne).val(),$(txt).val());
	layer.closeAll();

}

function getMyMoney(money,num,txt){
	if(My.color=="0"){layer.open({
		content: '请登录'
		,skin: 'msg'
		,time: 2 //2秒后自动关闭
	...
	$.getJSON("/index/ajax/mymoney",function(re){
		if(money>parseInt(re.money)){
			...
			$.getJSON("/ajax.php?act=sendhongbao",{rid:My.rid,money:money,number:num,txt:txt},
				function(re){
					if(re.code=="1"){
...
function openPPChatPtpChatWin(u,active){
	if($("#ptpChatLi"+u.chatid).length<1){
		...
		li.innerHTML='<a href="javascript:void(0)"  title="'+u.nick+'">'
			+'<span class="ugroup">'+groupimg+'</span>'
			+'<span class="uimg"><img src="'+u.mood+'" onerror="this.src=\'/style/face/p1/null.gif\'" border="0" class="head" /></span>'
...
function hdviewbind(){
	$(".viewhd").unbind();
	$(".viewhd").on("click",function(){openPopIfrmae('交易提示','/apps/hd.php')});
}
function app_sendmsg(str){
	$("#messageEditor").html(str);
	$("#sendBtn").click();
}
