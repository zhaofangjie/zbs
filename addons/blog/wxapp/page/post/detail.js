...
Page({
  ...
  onLoad: function (options) {
    ...
    app.request('/post/detail', { id: options.id }, function (data, ret) {
      ...
      that.setData({ postInfo: data.postInfo, commentList: data.commentList });
      that.page++;
    }, function (data, ret) {
      app.error(ret.msg);
    });
  ...
  formSubmit: function (event) {
    ...
    app.request('/comment/post', { post_id: this.data.postInfo.id, pid: this.data.form.quotepid, username: app.globalData.userInfo.nickName, avatar: app.globalData.userInfo.avatarUrl, content: event.detail.value.content }, function (data) {
      app.success('发表成功!');
